...
from pipeline.response import generate_remediation_command
...
def execute_ssh_isolation(host_ip: str, command: str):
    if not command:
        return
    ssh_user = "soc_admin"
    ...
    try:
        logging.info(f"Attempting SSH connection to: {host_ip}")
        ssh.connect(host_ip, username=ssh_user, password=ssh_password, timeout=10)
        stdin, stdout, stderr = ssh.exec_command(command)
        exit_status = stdout.channel.recv_exit_status()
        if exit_status == 0:
            logging.info(f"Command executed successfully on {host_ip}")
        else:
            logging.error(f"Command failed on {host_ip}: {stderr.read().decode()}")
    except Exception as e:
        logging.error(f"Failed to execute SSH response link to {host_ip}: {str(e)}")
    finally:
        ssh.close()
...
def send_interactive_telegram(alert: dict, ai_reason: str):
    ...
    callback_data = f"approve_{alert['attack_type']}_{alert['agent_ip']}_{alert['agent_name']}"
    ...
    reply_markup = {
        "inline_keyboard": [
            [
                {"text": "✅ Approve Action", "callback_data": callback_data},
    ...
    except Exception as e: logging.error(f"Telegram alerting failed: {e}")
...
async def telegram_interactive_callback(request: Request, background_tasks: BackgroundTasks):
    ...
    if callback_data.startswith("approve_"):
        ...
        action_map = {
             ...
             "credential_theft": "isolate_host",
             "c2": "isolate_host",
             "dropper": "quarantine"
        }
        chosen_action = action_map.get(attack_type, "monitor")
        
        rem_res = generate_remediation_command(chosen_action, target_ip=host_ip, target_host=host_name)
        exec_cmd = rem_res.get("command")
        
        if exec_cmd:
             background_tasks.add_task(execute_ssh_isolation, host_ip, exec_cmd)
             result_text = f"\n\n⚡ *RESPONSE EXECUTED:* Sent {chosen_action} command to `{host_name}` via SSH."
        else:
             result_text = f"\n\n🟡 *MONITORED:* Action resolved to monitor, no command generated."
    else:
